 ...
 return str.replace(/(^\s*)|(\s*$)/g, "");    
  ...
  function resetall(){
   ...
   var  pEditor  =  FCKeditorAPI.GetInstance("content");
	...
	<body bgcolor="#abcedf">
		<h3>
			<font style="font-size:12px">修改产品信息</font>
		</h3>
			<form name="productForm" method="POST"  enctype="multipart/form-data" onsubmit="return validate()" action="<%=path%>/product.do?cmd=updateProduct&id=${product.id}" style="font-size:12px">
				产品名称：<input type="text" name="title" size="30" maxlength="50" value="${product.title }">
				<br />
				...
				<FCK:editor instanceName="content" height="400" width="700">
					<jsp:attribute name="value">${product.content}</jsp:attribute>
				</FCK:editor>
